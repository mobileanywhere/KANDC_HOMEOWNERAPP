...
class _ViewAllServiceScreenState extends State<ViewAllServiceScreen> {
  ...
  void init() async {
    ...
    //   fetchCategoryList();
    // }
    getSubTypes(typeId: widget.categoryId ?? 0);
    filterServices("");
  }
  ...
  Widget staticSubCategoryWidget() {
    return Column(
      ...
      children: [
        ...
        //     .paddingLeft(16),
        // 16.height,
        mainSubTypesList.isEmpty
            ? NoDataWidget()
            : GridView.builder(
                shrinkWrap: true,
                itemCount: mainSubTypesList.length,
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                  crossAxisCount: 2,
                ),
                itemBuilder: (context, index) {
                  MainTypesData data = mainSubTypesList[index];
                  return SubCategoryComponentTwo(
                      categoryImage: data.typeImage.toString(),
                      categoryName: data.name.toString(),
                      onTap: () {
                        debugPrint('Sub-Type ID: ${data.id}');
                        setState(() {
                          subTypeId = data.id;
                        });
                        SubTypesScreen(
