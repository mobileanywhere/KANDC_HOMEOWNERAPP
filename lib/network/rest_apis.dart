...
import 'package:homeowner/model/city_list_model.dart';
import 'package:homeowner/model/country_list_model.dart';
import 'package:homeowner/model/dashboard_model.dart';
import 'package:homeowner/model/get_my_post_job_list_response.dart';
import 'package:homeowner/model/login_model.dart';
import 'package:homeowner/model/notification_model.dart';
import 'package:homeowner/model/post_job_detail_response.dart';
import 'package:homeowner/model/provider_info_response.dart';
import 'package:homeowner/model/provider_list_model.dart';
import 'package:homeowner/model/service_data_model.dart';
import 'package:homeowner/model/service_detail_response.dart';
import 'package:homeowner/model/service_response.dart';
import 'package:homeowner/model/service_review_response.dart';
import 'package:homeowner/model/state_list_model.dart';
import 'package:homeowner/model/user_data_model.dart';
import 'package:homeowner/model/verify_transaction_response.dart';
import 'package:homeowner/network/network_utils.dart';
...
Future<void> clearPreferences() async {
  ...
  await appStore.setSelectedPropertyName('');
  await appStore.setSelectedPropertyAddress('');
  await appStore.setSelectedPropertyAddressId('');
  await appStore.setLoggedIn(false);
  await removeKey(LOGIN_TYPE);

  // Please do not remove this condition because this feature not supported on iOS.
...
void _performAdditionalProcessing(DashboardResponse dashboardResponse) async {
  ...
  appStore.setPrivacyPolicy(
      // dashboardResponse.privacyPolicy?.value.validate() ??
      PRIVACY_POLICY_URL);
...
Future<CategoryResponse> getCategoryList(
    {required int id, String perPage = ''}) async {
  return CategoryResponse.fromJson(await handleResponse(await buildHttpResponse(
      'category-list?type_id=$id&per_page=$perPage',
      method: HttpMethodType.GET)));
}
  ...
  try {
    ...
    if (status == BOOKING_TYPE_ALL) {
      res = BookingListResponse.fromJson(await handleResponse(
          await buildHttpResponse(
              'booking-list?&customer_id=$customerId&per_page=$perPage&page=$page',
              method: HttpMethodType.GET)));
    } else {
      res = BookingListResponse.fromJson(await handleResponse(
          await buildHttpResponse(
...
Future<List<BookingData>> getInspectionList(int page, Map request,
    {var perPage = PER_PAGE_ITEM,
    String status = '',
    required List<BookingData> bookings,
    Function(bool)? lastPageCallback}) async {
  try {
    ...
    if (status == BOOKING_PAYMENT_STATUS_ALL) {
      res = BookingListResponse.fromJson(await handleResponse(
          await buildHttpResponse('inspections?per_page=$perPage&page=$page',
              request: request, method: HttpMethodType.POST)));
    } else {
      res = BookingListResponse.fromJson(await handleResponse(
          await buildHttpResponse(
              'inspections?status=$status&per_page=$perPage&page=$page',
              request: request,
              method: HttpMethodType.POST)));
    }
    ...
    cachedBookingListInspection = bookings;
...
Future<void> saveBooking(Map request, {List<File>? imageFile}) async {
  ...
  //   CommonKeys.bookingId: res[CommonKeys.bookingId].toString(),
...
Future<Map<String, String>> getMultipartFields(
    {required Map<dynamic, dynamic> val}) async {
...
Future<List<MainTypesData>> getMainTypes() async {
  MainTypesModel res = MainTypesModel.fromJson(
    await handleResponse(
        await buildHttpResponse('types', method: HttpMethodType.GET)),
  );
  List<MainTypesData> typesList = [];
  try {
    typesList.addAll(res.data ?? []);
  } catch (e) {
...
Future<List<MainTypesData>> getMainSubTypes({required int typeId}) async {
  MainTypesModel res = MainTypesModel.fromJson(
    await handleResponse(await buildHttpResponse('sub-types?type_id=$typeId',
        method: HttpMethodType.GET)),
  );
  List<MainTypesData> subTypesList = [];
  try {
    subTypesList.addAll(res.data ?? []);
  } catch (e) {
    debugPrint(e.toString());
  }
...
Future<PlanModel> getPlanList() async {
  // PlanModel res = PlanModel.fromJson(
  //   await handleResponse(
  //       ),
  // );
  var res = await buildHttpResponse('plan-list', method: HttpMethodType.GET);

  var re = PlanModel.fromJson(jsonDecode(res.body));

  // List<PlanData> subTypesList = [];
  // try {
  //   debugPrint("length of plan list: ${re.data.toString()}");
  //   subTypesList.addAll(re.data ?? []);
  //   debugPrint("length of plan list: ${subTypesList.length.toString()}");
  // } catch (e) {
  //   debugPrint(e.toString());
  // }
...
Future<PurchasePlanModel> purchasePlan(
    {required Map<String, dynamic> request}) async {
  var res = PurchasePlanModel.fromJson(await handleResponse(
    await buildHttpResponse(
      'purchase_plan',
      method: HttpMethodType.POST,
      request: request,
...
Future<EstimationDetailsResponse> estimationDetails(Map request) async {
  var response = await buildHttpResponse('estimation_detail',
      request: request, method: HttpMethodType.POST);
  return EstimationDetailsResponse.fromJson(response.body);
}

Future<void> approveEstimation(Map request) async {
  try {
    var res = BaseResponseModel.fromJson(await handleResponse(
        await buildHttpResponse('appove_estimation',
            request: request, method: HttpMethodType.POST)));
...
Future<void> declineEstimation(Map request) async {
  try {
    var res = BaseResponseModel.fromJson(await handleResponse(
        await buildHttpResponse('decline_estimation',
            request: request, method: HttpMethodType.POST)));
...
Future<void> addProperty(Map request) async {
  try {
    var res = BaseResponseModel.fromJson(await (handleResponse(
        await buildHttpResponse('add-property',
            request: request, method: HttpMethodType.POST))));
    toast(res.message);
  } catch (e) {
    debugPrint(e.toString());
...
Future<void> editProperty(Map request) async {
  try {
    var res = BaseResponseModel.fromJson(await (handleResponse(
        await buildHttpResponse('edit-property',
            request: request, method: HttpMethodType.POST))));
    toast(res.message);
  } catch (e) {
    debugPrint(e.toString());
...
Future<List<PropertyData>> getAllProperties(Map req) async {
  PropertyModel res = PropertyModel.fromJson(
    await handleResponse(await buildHttpResponse('property',
        request: req, method: HttpMethodType.POST)),
  );
  List<PropertyData> propertiesList = [];
  try {
    propertiesList.addAll(res.data ?? []);
  } catch (e) {
    debugPrint(e.toString());
  }
...
Future<List<PropertyData>> getAllPropertiesBySubscription(Map req) async {
  PropertyModel res = PropertyModel.fromJson(
    await handleResponse(await buildHttpResponse('property-by-subscription',
        request: req, method: HttpMethodType.POST)),
  );
  List<PropertyData> propertiesList = [];
  try {
...
//region Payment Api
Future<BaseResponseModel> updatePayment(Map request) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('update-payment',
          request: request, method: HttpMethodType.POST)));
}

Future<InvoiceDataModel> getInvoice(Map request) async {
  return InvoiceDataModel.fromJson(await handleResponse(await buildHttpResponse(
      'invoice',
      request: request,
      method: HttpMethodType.POST)));
}

Future<BaseResponseModel> paymentForInvoice(Map request) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('save-payment',
          request: request, method: HttpMethodType.POST)));
}
