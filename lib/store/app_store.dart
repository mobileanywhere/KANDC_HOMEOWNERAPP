...
abstract class _AppStore with Store {
  ...
  @observable
  String selectedPropertyAddressId = '';

  @action
  Future<void> setSelectedPropertyName(String val,
      {bool isInitializing = false}) async {
    selectedPropertyName = val;
    if (!isInitializing) await setValue(SELECTED_PROPERTY_NAME, val);
  }

  @action
  Future<void> setSelectedPropertyAddress(String val,
      {bool isInitializing = false}) async {
    selectedPropertyAddress = val;
    if (!isInitializing) await setValue(SELECTED_PROPERTY_ADDRESS, val);
  }

  @action
  Future<void> setSelectedPropertyAddressId(String val,
      {bool isInitializing = false}) async {
    selectedPropertyAddressId = val;
    if (!isInitializing) await setValue(SELECTED_PROPERTY_ADDRESS_ID, val);
  }
